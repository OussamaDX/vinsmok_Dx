...
/*   Updated: 2023/07/13 22:29:41 by ooussaad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start_game(t_game_mape ptr, char *holder)
{
	...
	position(ptr.map, 'P');
	ptr.mlx_window = mlx_new_window(ptr.mlx_pointr, len_with(ptr.map22) * 50,
			len_hight(ptr.map22) * 50, "so_long_dx");
	ft_wall(&ptr);
	// ft_coins(&ptr);
	// ft_player(&ptr);
	// ft_exit(&ptr);
